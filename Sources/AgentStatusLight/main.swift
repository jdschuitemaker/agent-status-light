...
@MainActor final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func refresh() { pulse += 0.24; for source in sources { let old = states[source] ?? .off; let (new, updated) = statusRecord(source); if old != new { states[source] = new }; if new == .failed && old != new && pref("playFailureSound") { failure(source) }; if new == .awaitingInput && pref("playInputSound") { let last = awaitAlertedAt[source] ?? .distantPast; let freshAwait = updated.map { $0 > last } ?? false; if old != new || freshAwait { ping(source); awaitAlertedAt[source] = updated ?? Date() } }; if new == .working || old != new { render(source) } } }
    ...
    @objc private func closeSelf(_ sender: NSMenuItem) { guard let source = sender.representedObject as? String, sources.count > 1 else { return }; sources.removeAll { $0 == source }; UserDefaults.standard.set(sources, forKey: "visibleSources"); if let item = items.removeValue(forKey: source) { NSStatusBar.system.removeStatusItem(item) }; let ids = buttonSources.filter { $0.value == source }.map(\.key); for id in ids { buttonSources.removeValue(forKey: id) }; states.removeValue(forKey: source); players.removeValue(forKey: source); pings.removeValue(forKey: source); awaitAlertedAt.removeValue(forKey: source) }
