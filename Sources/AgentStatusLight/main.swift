...
struct SessionRecord: Codable { var state: AgentState; var updatedAt: Date; var source: String?; var session: String?; var label: String? }
struct SessionInfo { let key: String; let label: String; let state: AgentState; let updatedAt: Date; let url: URL }
...
@MainActor final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @objc private func closeSelf(_ sender: NSMenuItem) { guard let source = sender.representedObject as? String, sources.count > 1 else { return }; sources.removeAll { $0 == source }; UserDefaults.standard.set(sources, forKey: "visibleSources"); if let item = items.removeValue(forKey: source) { NSStatusBar.system.removeStatusItem(item) }; let ids = buttonSources.filter { $0.value == source }.map(\.key); for id in ids { buttonSources.removeValue(forKey: id) }; states.removeValue(forKey: source); players.removeValue(forKey: source); pings.removeValue(forKey: source); awaitAlertedAt.removeValue(forKey: source); sessions.removeValue(forKey: source); sessionAlertedAt = sessionAlertedAt.filter { !$0.key.hasPrefix(source + "|") } }
