...
@MainActor final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func refresh() { pulse += 0.24; tickCount += 1; loadSessions(); for source in sources { let old = states[source] ?? .off; let (new, updated) = aggregate(source); states[source] = new; let list = sessions[source] ?? []; if list.isEmpty { if new == .failed && old != new && pref("playFailureSound") { failure(source) }; if new == .awaitingInput && pref("playInputSound") { let last = awaitAlertedAt[source] ?? .distantPast; let freshAwait = updated.map { $0 > last } ?? false; let settled = updated.map { Date().timeIntervalSince($0) >= 0.25 } ?? true; if settled && (old != new || freshAwait) { ping(source); awaitAlertedAt[source] = updated ?? Date() } } } else { for s in list { let key = "\(source)|\(s.key)"; if s.state == .failed { let last = sessionAlertedAt[key] ?? .distantPast; if s.updatedAt > last { sessionAlertedAt[key] = s.updatedAt; if pref("playFailureSound") { failure(source) } } } else if s.state == .awaitingInput { let last = sessionAlertedAt[key] ?? .distantPast; if s.updatedAt > last && Date().timeIntervalSince(s.updatedAt) >= 0.25 { sessionAlertedAt[key] = s.updatedAt; if pref("playInputSound") { ping(source) } } } } }; if new == .working || old != new { render(source) } } }
